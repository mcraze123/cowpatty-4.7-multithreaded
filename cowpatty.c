...
void *cowpatty_hash_worker(void *args);
void *cowpatty_dictfile_worker(void *args);
	...
	struct crack_data *cdata)
{
	...
	/* Get the number of CPU's on this system for optimal thread count */
	int num_cpu = sysconf(_SC_NPROCESSORS_ONLN) + 1;

	//pthread_t hashfile_worker[num_cpu];
	...
	while (feof(fp) == 0 && sig == 0) {
		for(i = 0; i < num_cpu; i++){
			...
			//reclen = nexthashrec(fp, &rec); /* casuses set but not used gcc warning */
			nexthashrec(fp, &rec);
			...
			passphrase[wordlen] = '\0'; /* changed from 0 to '\0' for consistency */
...
// set a global flag if password is found in a thread, check at begining of loop and do this return
...
/* Hash file attack thread worker */
void *cowpatty_hash_worker(void *args){
	pthread_exit(NULL);
}
...
void cowpatty_thread_worker(void *args){
	pthread_exit(NULL);
}
